    ...
    <section class="site-banner mt-8">
        ...
        <div class="container py-lg-5 shadow borderDewandik px-4">
            <div class="row justify-content-center">
                <div class="col-12">
                    <form action="<?= site_url('index/search_video') ?>" method="get">
                ...
                <?php if ($hasil === 'Tidak ditemukan') : ?>
                    ...
                    <div class="col-md justify-content-center">
                        ...
                        <div class="card-body pt-0 mt-4 text-center">
                            ...
                            <a href="<?= base_url('video'); ?>" class="btn btn-primary text-center">&larr; Kembali</a>
                        </div>
                    </div>
                <?php else : ?>
                    <?php foreach ($data->result() as $rowResult) : ?>
                        <div class="col-lg-3 ml-1 mr-1 mt-2 mb-3 col-md-6 col-sm-12">
                            <div class="card h-100 border-light shadow">
                                <img class="rounded mx-3 mt-3" src="<?= thumbYouTube(IDYouTube($rowResult->link)) ?>" alt="<?= $rowResult->judul_vid ?>">
                                <div class="card-body">
                                    <p class="card-text"><i class="text-primary fad fa-calendar"></i> <?= $rowResult->tanggal_vid ?></p>
                                    <h6 class="card-title"><b><?= $rowResult->judul_vid ?></b>: <?= word_limiter(strip_tags($rowResult->deskripsi_vid), 10) ?></h6>
                                </div>
                                <div class="card-footer text-center">
                                    <a href="<?= site_url('index/detail_video/' . $rowResult->id_video); ?>" class="btn btn-primary btn-md m-2 fw-bold text-center">Lihat Detail</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
